...
function renderTeamsPage() {
  ...
  if (!userTeams.length) {
    ...
    for (var i = 0; i < userTeams.length; i++) {
      ...
      h += '<button style="padding:8px 12px;border-radius:10px;border:1px solid rgba(255,71,87,.25);background:transparent;color:#ff4757;font-size:12px;cursor:pointer;transition:all .2s" onclick="leaveTeam(\'' + team.id + '\')">' + (isOwner ? '🗑' : '🚪') + '</button>';
      h += '</div></div>';
