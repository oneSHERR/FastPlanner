...
function listenTeamsList(uid) {
  ...
  unsubTeamsList = db.collection("teams").where("memberIds", "array-contains", uid)
    .onSnapshot(function(snap) {
      ...
      var seenIds = {};
      snap.forEach(function(doc) {
        if (seenIds[doc.id]) return; // skip duplicates
        seenIds[doc.id] = true;
        var d = doc.data();
        d.id = doc.id;
        userTeams.push(d);
...
var _createTeamTime = 0;
window.createTeam = function() {
  // Debounce: ignore if called within 3 seconds of last create
  var now = Date.now();
  if (_creatingTeam || now - _createTeamTime < 3000) return;
...
window.joinTeam = function() {
  var now = Date.now();
  if (_joiningTeam || now - _joinTeamTime < 3000) return;
...
var _renderTeamsVersion = 0;

function renderTeamsPage() {
  _renderTeamsVersion++;
  var myVersion = _renderTeamsVersion;
  ...
  if (allUids.length > 0) {
    ...
    Promise.all(promises).then(function(snaps) {
      if (myVersion !== _renderTeamsVersion) return; // stale, skip
      memberOnlineStatus = {};
      ...
      if (myVersion !== _renderTeamsVersion) return;
      renderTeamsPageInner();
    });
  } else {
    renderTeamsPageInner();
...
function renderTeamsPageInner() {
  ...
  // Deduplicate userTeams by id (safety net)
  var seen = {}, clean = [];
  for (var i = 0; i < userTeams.length; i++) {
    var tid = userTeams[i].id;
    if (tid && !seen[tid]) { seen[tid] = true; clean.push(userTeams[i]); }
  }
  if (clean.length !== userTeams.length) {
    console.warn("Teams dedup: had " + userTeams.length + " → " + clean.length);
  }
  userTeams = clean;
