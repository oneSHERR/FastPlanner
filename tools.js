...
function renderTools() {
  ...
  // ---- Tool 1: Word splitter ----
  h += '<div class="tcard" style="background:' + T.cbg + ';border:1px solid ' + T.cb + ';margin-bottom:20px">';
  ...
  // ---- Tool 2: Container joiner ----
  h += '<div class="tcard" style="background:' + T.cbg + ';border:1px solid ' + T.cb + '">';
  h += '<h3 style="font-size:18px;font-weight:700;margin-bottom:6px;display:flex;align-items:center;gap:8px;color:' + T.tx + ';letter-spacing:-.01em">📦 Соединитель контейнеров</h3>';
  h += '<p style="font-size:13px;margin-bottom:18px;color:' + T.tm + ';letter-spacing:.01em">Убирает пробелы, запятые и лишние символы между префиксом и номером контейнера.<br>Например: <span style="font-family:\'JetBrains Mono\',monospace;color:' + T.at + '">TKRU 4236025</span> → <span style="font-family:\'JetBrains Mono\',monospace;color:#2ed573">TKRU4236025</span></p>';

  h += '<div class="trow">';
  h += '<div class="tcol"><label style="color:' + T.tm + ';font-size:11px;text-transform:uppercase;letter-spacing:.08em;margin-bottom:6px;display:block;font-weight:600">Вставьте контейнеры</label>';
  h += '<textarea class="tta" id="ctk-inp" style="border:1px solid ' + T.ibd + ';background:' + T.ib + ';color:' + T.tx + '" placeholder="TKRU 4236025\nTKRU 4236026, \nMSKU 1234567"></textarea></div>';
  h += '<div class="tcol"><label style="color:' + T.tm + ';font-size:11px;text-transform:uppercase;letter-spacing:.08em;margin-bottom:6px;display:block;font-weight:600">Результат</label>';
  h += '<textarea class="tta" id="ctk-out" style="border:1px solid ' + T.ibd + ';background:' + T.ib + ';color:' + T.tx + '" readonly placeholder="Результат"></textarea>';
  h += '<div id="ctk-cnt" style="font-size:12px;margin-top:8px;color:' + T.td + ';letter-spacing:.02em"></div></div></div>';

  h += '<div style="display:flex;gap:10px;margin-top:18px;flex-wrap:wrap">';
  h += '<button class="tbtn btn-glow" style="background:linear-gradient(135deg,' + T.ac + ',' + T.ac + 'cc);box-shadow:0 4px 20px ' + T.ag + '" onclick="runCtk()">Соединить</button>';
  h += '<button class="tbtn2" style="background:' + T.ab + ';color:' + T.at + '" onclick="copyCtk()">📋 Скопировать</button>';
  h += '<button class="tbtn2" style="background:' + T.chb + ';color:' + T.tm + '" onclick="clearCtk()">Очистить</button>';
  h += '<button class="tbtn2" style="background:' + T.chb + ';color:' + T.tm + '" onclick="demoCtk()">Демо</button>';
  h += '</div></div>';
...
// TOOL 2 — Container Joiner
// ============================================================
window.runCtk = function() {
  var inp = document.getElementById("ctk-inp");
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    // Remove trailing commas, semicolons, dots, spaces
    line = line.replace(/[,;.\s]+$/g, "");

    // Remove ALL spaces, tabs, invisible chars within the line
    line = line.replace(/[\s\u00A0\u200B\u200C\u200D\uFEFF\u2000-\u200A\u2028\u2029\u202F\u205F\u3000]+/g, "");

    if (line) res.push(line.toUpperCase());
  }

  var out = document.getElementById("ctk-out"), cnt = document.getElementById("ctk-cnt");
  if (out) out.value = res.join("\n");
  if (cnt) cnt.textContent = "Контейнеров: " + res.length;
};

window.copyCtk = function() {
  var o = document.getElementById("ctk-out");
  if (o && o.value) navigator.clipboard.writeText(o.value).catch(function() { o.select(); document.execCommand("copy"); });
};

window.clearCtk = function() {
  var i = document.getElementById("ctk-inp"), o = document.getElementById("ctk-out"), c = document.getElementById("ctk-cnt");
  if (i) i.value = ""; if (o) o.value = ""; if (c) c.textContent = "";
};

window.demoCtk = function() {
  var i = document.getElementById("ctk-inp");
  if (i) i.value = "TKRU 4236025\nTKRU 4236026, \nMSKU 123 4567\nGATU  9876543,\nFFAU 1112 233";
  runCtk();
};
