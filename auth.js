...
function saveUserAvatar(dataUrl) {
  ...
  db.collection("users").doc(currentUser.uid).set({avatarUrl: dataUrl}, {merge: true}).then(function() {
    // Update avatar in all teams this user belongs to
    updateAvatarInTeams(dataUrl);
    renderProfile();
    renderSidebar();
  }).catch(function(err) { alert("Ошибка: " + err.message); });
}

function updateAvatarInTeams(avatarUrl) {
  ...
  for (var i = 0; i < userTeams.length; i++) {
    ...
    var members = (team.members || []).map(function(m) {
      if (m.uid === uid) { updated = true; return Object.assign({}, m, {avatarUrl: avatarUrl}); }
      return m;
    });
    if (updated) {
...
function renderAuth() {
  ...
  if (authMode === "login") {
    ...
    h += '<input class="auth-input" id="a-email" style="border:1px solid ' + T.ibd + ';background:' + T.ib + ';color:' + T.tx + '" placeholder="Email" type="email">';
    h += '<input class="auth-input" id="a-pass" style="border:1px solid ' + T.ibd + ';background:' + T.ib + ';color:' + T.tx + '" placeholder="Пароль (мин. 6 символов)" type="password" onkeydown="if(event.key===\'Enter\')doRegister()">';
...
window.doRegister = function() {
  ...
  var email = (document.getElementById("a-email") || {}).value || "";
  var pass = (document.getElementById("a-pass") || {}).value || "";
  if (!name || !email || !pass) { showAuthError("Заполните все поля"); return; }
