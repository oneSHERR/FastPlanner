...
window.switchTab = function(tab) {
  // Stop game loops when leaving
  if (activeTab === "game" && tab !== "game") stopGameLoop();
  if (activeTab === "defense" && tab !== "defense") stopDefenseLoop();

  activeTab = tab;
  var panels = ["planner", "teams", "tools", "game", "defense", "profile"];
  var order = {planner:0, teams:1, tools:2, game:3, defense:4, profile:5};
  var idx = order[tab] || 0;
  ...
  if (tab === "defense") { renderDefensePage(); }
};
...
function renderSidebar() {
  ...
  var tabs = [
    ...
    {id:"defense", icon:"🛡️", tip:"Space Defense"},
