...
function renderProfile() {
  ...
  // Teams info
  if (userTeams.length) {
    h += '<div style="margin-top:18px;padding:16px 20px;background:' + T.chb + ';border:1px solid ' + T.cb + ';border-radius:14px;backdrop-filter:blur(10px)">';
    h += '<p style="font-size:12px;color:' + T.td + ';margin-bottom:8px">Мои команды</p>';
    for (var i = 0; i < userTeams.length; i++) {
      var tm = userTeams[i];
      h += '<div style="display:flex;align-items:center;gap:8px;padding:6px 0;' + (i > 0 ? 'border-top:1px solid ' + T.cb : '') + '">';
      h += '<span style="font-size:14px">👥</span>';
      h += '<span style="font-size:13px;font-weight:600;color:' + T.tx + ';flex:1">' + esc(tm.name) + '</span>';
      h += '<span style="font-size:11px;font-family:\'JetBrains Mono\',monospace;color:' + T.at + ';letter-spacing:.08em">' + esc(tm.code) + '</span>';
      h += '</div>';
    }
    h += '</div>';
  }

  // Quick actions (useful on mobile where sidebar buttons are hidden)
  h += '<div style="margin-top:18px;display:flex;gap:10px;flex-wrap:wrap">';
  h += '<button style="flex:1;padding:14px;border-radius:14px;border:1px solid ' + T.cb + ';background:' + T.chb + ';color:' + T.tx + ';font-size:14px;font-weight:600;cursor:pointer;transition:all .2s;display:flex;align-items:center;justify-content:center;gap:8px;backdrop-filter:blur(10px)" onclick="openTP(event)">🎨 Сменить тему</button>';
  h += '<button style="flex:1;padding:14px;border-radius:14px;border:1px solid rgba(255,71,87,.25);background:rgba(255,71,87,.06);color:#ff4757;font-size:14px;font-weight:600;cursor:pointer;transition:all .2s;display:flex;align-items:center;justify-content:center;gap:8px" onclick="doLogout()">🚪 Выйти</button>';
  h += '</div>';
...
window.updateDisplayName = function() {
  ...
  currentUser.updateProfile({displayName: newName}).then(function() {
    ...
    // Update name in teams
    updateNameInTeams(newName);
    renderProfile();
    renderPlanner();
    renderSidebar();
...
function updateNameInTeams(newName) {
  if (!currentUser || !userTeams) return;
  var uid = currentUser.uid;
  for (var i = 0; i < userTeams.length; i++) {
    var team = userTeams[i];
    var updated = false;
    var members = (team.members || []).map(function(m) {
      if (m.uid === uid) { updated = true; return Object.assign({}, m, {name: newName}); }
      return m;
    });
    if (updated) {
      db.collection("teams").doc(team.id).update({members: members});
