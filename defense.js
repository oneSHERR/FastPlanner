...
function getShopItems() {
  ...
  return [
    ...
    {id:"homing", name:"🎯 Наведение ур." + (u.homing+1), desc:"Пули летят к врагам", cost: 35 + u.homing * 30, max: u.homing >= 5, action:function(){ u.homing++; }},
    {id:"pierce", name:"⚡ Пробивание ур." + (u.piercing+1), desc:"Пули сквозь врагов", cost: 40 + u.piercing * 35, max: u.piercing >= 4, action:function(){ u.piercing++; }},
...
function coopJoinRoom(code) {
  ...
  db.collection("defense-rooms").where("code", "==", code.toUpperCase()).where("status", "==", "waiting").limit(1).get()
    .then(function(snap) {
      if (snap.empty) { dfLobbyError = "Комната не найдена или уже занята"; dfLobbyMsg = ""; return; }
      var doc = snap.docs[0], data = doc.data();
      if (data.hostUid === currentUser.uid) { dfLobbyError = "Нельзя присоединиться к своей комнате!"; dfLobbyMsg = ""; return; }
      if (data.guestUid) { dfLobbyError = "Комната уже заполнена!"; dfLobbyMsg = ""; return; }
...
function coopNotifyShop() {
  ...
  dfRoomRef.update({ status: "shop", upgrades: JSON.stringify(dfUpgrades), coins: dfCoins }).catch(function(){});
}
function coopNotifyNextWave() {
  if (!dfCoop || !dfIsHost || !dfRoomRef) return;
  dfRoomRef.update({ status: "playing", wave: dfWave, upgrades: JSON.stringify(dfUpgrades), coins: dfCoins }).catch(function(){});
}
...
function updateDefense() {
  ...
  if(dfCoop&&dfIsHost&&dfPartnerBullets.length>0){
    for(var i=0;i<dfPartnerBullets.length;i++){
      ...
      for(var j=dfEnemies.length-1;j>=0;j--){
        var e=dfEnemies[j];
        if(Math.hypot(pb.x-e.x,pb.y-e.y)<e.radius+6){e.hp-=1;break;}
      }
    }
  }
...
function drawDefense() {
  ...
  // My ship
  if(dfUpgrades.shipHP>0&&(dfInvincible<=0||Math.floor(dfInvincible/4)%2===0)) drawPixelShip(ctx,dfShip.x,dfShip.y,dfShip.angle,30,dfShip.thrust);
...
function drawShop() {
  ...
  ctx.font="16px Inter";ctx.fillStyle="#2ed573";ctx.fillText("Монеты: "+dfCoins,dfW/2,78);
  ctx.font="14px Inter";ctx.fillStyle="#aaa";ctx.fillText("Волна "+dfWave+" пройдена! Очки: "+dfScore,dfW/2,100);
  if(dfCoop){ctx.font="bold 12px Inter";ctx.fillStyle="#ff9f43";ctx.fillText("👥 Кооператив с "+(dfPartnerName||"напарником"),dfW/2,118);}
...
function renderDefensePage() {
  ...
  dfCanvas.addEventListener("mousedown",function(e){
    dfMouseDown=true;
    var r=dfCanvas.getBoundingClientRect(),mx=e.clientX-r.left,my=e.clientY-r.top;
  ...
  // Touch
  dfCanvas.addEventListener("touchstart",function(e){e.preventDefault();var t=e.touches[0],r=dfCanvas.getBoundingClientRect();dfMouseX=t.clientX-r.left;dfMouseY=t.clientY-r.top;dfMouseDown=true;if(dfState==="menu"){dfCoop=false;initDefenseGame();}});
  dfCanvas.addEventListener("touchmove",function(e){e.preventDefault();var t=e.touches[0],r=dfCanvas.getBoundingClientRect();dfMouseX=t.clientX-r.left;dfMouseY=t.clientY-r.top;});
  ...
  document.addEventListener("keydown",function(e){
    ...
    if(dfState==="shop"&&e.key>="1"&&e.key<="9"){
      ...
      if(idx<items.length&&!items[idx].max&&dfCoins>=items[idx].cost){dfCoins-=items[idx].cost;items[idx].action();if(dfCoop&&dfIsHost)dfRoomRef.update({coins:dfCoins,upgrades:JSON.stringify(dfUpgrades)}).catch(function(){});}
    }
  });
  ...
  dfCanvas.addEventListener("click",function(e){
    ...
    for(var i=0;i<items.length;i++){
      ...
      if(mx>=x&&mx<=x+itemW&&my>=y&&my<=y+itemH){
        if(!items[i].max&&dfCoins>=items[i].cost){dfCoins-=items[i].cost;items[i].action();if(dfCoop&&dfIsHost)dfRoomRef.update({coins:dfCoins,upgrades:JSON.stringify(dfUpgrades)}).catch(function(){});}
        break;
      }
    }
  });
